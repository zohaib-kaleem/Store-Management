...
public class UpdateCustomerController implements SceneManager.DataReceiver<User> {
    ...
    public void delete() {
        UserService userService = new UserService();
        try {
            userService.removeUser(userToUpdate.getUsername(), "customer");

            MessageUtil.showMessage("Store", "Customer removed successfully.");
            goBack();

        } catch (Exception e) {
            MessageUtil.showError("Update Customer", e.getMessage());
        }
    }
